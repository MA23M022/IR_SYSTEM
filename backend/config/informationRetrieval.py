...
from backend.utils.lsa import LSA
from backend.utils.esa import ESARetrieval
from backend.utils.dbESA import DBESA
from backend.utils.concept_matrix_computation import load_esa_model

term_concept_matrix, vocab = load_esa_model(prefix='backend/model/cranfield_esa_model')

class InformationRetrieval():
    
    def __init__(self, method="tfidf"):
        assert method in ["tfidf", "lsa", "esa", "dbesa"], "method must be 'tfidf' or 'lsa'"
        self.method = method
        self.docIDs = None

        if self.method == "lsa":
            print("Calling lsa")
            self.model = LSA(n_components=500)  # 900
        elif method == "esa":
            print("calling esa")
    ...
    def rank(self, queries):
        ...
        """
        if self.method in ["lsa" ,"esa", "dbesa"]:
